...
public class LlmHubService {
    ...
    @Value("${llmhub.default-provider}")
    private String defaultProvider;

    public Flux<String> streamDispatch(RequestContext context) {
        ...
        byte[] upstreamBodyBytes = JsonUtil.writeValueAsBytes(context.request.openai);
    ...
    private void route(RequestContext context) {
        ...
        Provider provider = providerName == null || providers == null
                ? null
        ...
        Map<String, Object> paramOverride = provider.getParamOverride();
        if (paramOverride != null) {
            Object modelOverride = paramOverride.get("model");
            if (modelOverride instanceof Map) {
                // modelOverride = { overrideFrom, overrideTo }; `*` is the wildcard
                Map<?, ?> overrides = (Map<?, ?>) modelOverride;
                Object override = modelName != null ? overrides.get(modelName) : null;
                if (override == null) {
                    override = overrides.get("*");
                }
                if (override != null && !override.toString().isEmpty()) {
                    modelName = override.toString();
                }
            }
        }
    ...
    private void buildUpstreamHeaders(HttpHeaders headers, RequestContext context) {
        Provider provider = context.trueProvider;
    ...
    private String buildResponseFrame(RequestContext context, AtomicBoolean firstFrameSent, long providerRecvTime, String rawChunk) {
    ...
    private Map<String, Object> buildTrace(RequestContext context, AtomicBoolean firstFrameSent, long recvFromProviderTime) {
        ...
        if (firstFrameSent.compareAndSet(false, true)) {
            ...
            trace.put("requestAt", TimeUtil.toUtcString(context.startTime));
            trace.put("sendToProviderAt", TimeUtil.toUtcString(context.sendToProviderTime));
        }
        trace.put("recvFromProviderAt", TimeUtil.toUtcString(recvFromProviderTime));
        trace.put("responseAt", TimeUtil.toUtcString(System.currentTimeMillis()));
        return trace;
    }
